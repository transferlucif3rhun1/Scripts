...
if (cluster.isMaster) {
  ...
  app.post("/challenge", async (req, res) => {
    ...
    try {
      const { proxy, email, password } = req.body || {};
      ...
      const setCookies = solveResponse.headers?.["Set-Cookie"];
      ...
      const config = {
        headers: {
          ...
          "sec-ch-ua-platform": '"Windows"',
          "next-action": "7fdcf3f1186e6a410fe530f327ce0a3ddfc2a9f793",
          "sec-ch-ua": '"Google Chrome";v="131", "Chromium";v="131", "Not_A Brand";v="24"',
          "sec-ch-ua-mobile": "?0",
          "next-router-state-tree": "%5B%22%22%2C%7B%22children%22%3A%5B%5B%22locale%22%2C%22en%22%2C%22d%22%5D%2C%7B%22children%22%3A%5B%22(main)%22%2C%7B%22liveFeed%22%3A%5B%22__PAGE__%22%2C%7B%22children%22%3A%5B%22__PAGE__%22%2C%7B%7D%2C%22%2Fen%22%2C%22refresh%22%5D%7D%2C%22%2Fen%22%2C%22refresh%22%5D%2C%22children%22%3A%5B%22__PAGE__%22%2C%7B%7D%2C%22%2Fen%22%2C%22refresh%22%5D%7D%5D%7D%2Cnull%2Cnull%2Ctrue%5D%7D%5D",
          "user-agent": useragent,
          "accept": "text/x-component",
          "dnt": "1",
          "content-type": "text/plain;charset=UTF-8",
          "x-deployment-id": "dpl_HMpPYdvGuohdxXSe1pzxUwAAQJ8U",
          "sec-gpc": "1",
          "accept-language": "en-US,en;q=0.9",
          "origin": "https://packdraw.com",
          "sec-fetch-site": "same-origin",
          "sec-fetch-mode": "cors",
          "sec-fetch-dest": "empty",
          "referer": "https://packdraw.com/en",
          "accept-encoding": "gzip, deflate, br, zstd",
          "priority": "u=1, i"
        },
        headerOrder: [
          "content-length", "sec-ch-ua-platform", "next-action", "sec-ch-ua",
          "sec-ch-ua-mobile", "next-router-state-tree", "user-agent", "accept",
          "dnt", "content-type", "x-deployment-id", "sec-gpc", "accept-language",
          "origin", "sec-fetch-site", "sec-fetch-mode", "sec-fetch-dest",
          "referer", "accept-encoding", "priority"
        ],
        requestUrl: "https://packdraw.com/en",
        requestMethod: "POST",
        requestBody: payload,
      ...
      const finalResponse = await makeRequest(createPayload(config, sessionId));
      
      if (finalResponse.status === 403) {
        throw new Error("Request blocked after authentication");
      }
      if (sessionId) {
        sessions.delete(sessionId);
        console.log(`[WORKER-${process.pid}] Session ${sessionId} cleaned up after successful response`);
      }
      
      res.status(finalResponse.status || 200).json(finalResponse);
      
    } catch (error) {
      if (sessionId) {
        sessions.delete(sessionId);
        console.log(`[WORKER-${process.pid}] Session ${sessionId} cleaned up after error`);
      }
