      ...
      gojs: {
        ...
        "syscall/js.valueGet": (v_ref, p_ptr, p_len) => {
          ...
          if (prop === "importScripts") {
            return boxValue((_path) => {});
          }

          // checks if we are actually running in browser
          if (prop === "process" || prop === "require") {
            return boxValue(undefined);
          }

          if (prop === "createImageBitmap") {
            return boxValue((_image) => {});
          }

          // this is a check for dev tools
          if (prop === "d") {
            return boxValue(() => {
              return false;
            });
          ...
          if (prop === "e") {
            return boxValue((a) => {
              if (a.startsWith("function g(J")) {
                return new Promise((resolve) => {
                  // extract dynamic value from script
                  resolve(new RegExp("C='([^']+)'").exec(a)[1]);
                });
              }
