const express = require('express')
const http = require('http')
const cors = require('cors')
const axios = require('axios')

const app = express()

const server = http.createServer(app)

app.use(cors())
app.use(express.urlencoded({ extended: true }))
app.use(express.json())



//-------------------- CONSTANTS --------------------//


let C = null
let aa = null
let fa = null

let platforms = [
    "Win32", 
    //"MacIntel"
]

let screens = [
    "1068:970:1920:1040:1920:1040"
]

let plugins = [
    "p,Chrome PDF Viewer,Chromium PDF Viewer,Microsoft Edge PDF Viewer,PDF Viewer,WebKit built-in PDF"
]

let fingerprints = [
    "1080x1920 24"
]

let video = [
    "fv,ogg,mp4,webm"
]

let audio = [
    "fa,mpeg,ogg,wav"
]

let signatures = [
    "data:image/png;base64,"
]

let touch = [
    JSON.stringify({"mtp":0,"ts":false,"te":false})
]

let cookie = [
    JSON.stringify({"tc":true,"nc":true})
]

let webGL = [
    JSON.stringify({"ContextName":"webgl","VENDOR":"WebKit","VERSION":"WebGL 1.0 (OpenGL ES 2.0 Chromium)","RENDERER":"WebKit WebGL","SHADING_LANGUAGE_VERSION":"WebGL GLSL ES 1.0 (OpenGL ES GLSL ES 1.0 Chromium)","DEPTH_BITS":24,"MAX_VERTEX_ATTRIBS":16,"MAX_VERTEX_TEXTURE_IMAGE_UNITS":16,"MAX_VARYING_VECTORS":30,"MAX_VERTEX_UNIFORM_VECTORS":4095,"MAX_COMBINED_TEXTURE_IMAGE_UNITS":32,"MAX_TEXTURE_SIZE":16384,"MAX_CUBE_MAP_TEXTURE_SIZE":16384,"MAX_RENDERBUFFER_SIZE":16384,"MAX_VIEWPORT_DIMS":{"0":32767,"1":32767},"ALIASED_LINE_WIDTH_RANGE":{"0":1,"1":1},"ALIASED_POINT_SIZE_RANGE":{"0":1,"1":1024}})
]

let fonts = [
    "620.05078125,620.05078125,620.05078125,620.05078125,647.75390625,791.9296875,620.05078125,620.05078125,647.75390625,620.05078125,620.05078125,620.05078125,620.05078125,620.05078125,624.7265625,658.16015625,658.16015625,620.05078125,620.05078125,620.05078125,620.05078125,618.9609375,514.6171875,561.69140625,561.69140625,620.05078125,620.05078125,696.515625,647.75390625,620.05078125,614.28515625,620.05078125,620.05078125,563.9765625,620.05078125,620.05078125,620.05078125,620.05078125,620.05078125,734.625,649.01953125,620.05078125,620.05078125,468,620.05078125,574.3125,620.05078125,649.01953125,620.05078125,620.05078125,620.05078125,620.05078125,698.484375,802.0546875,815.34375,672.46875,635.9765625,694.16015625,672.46875,654.328125,620.05078125,620.05078125,620.05078125,660.65625,759.5859375,849.55078125,620.05078125,620.05078125"
]


//-------------------- CONSTANTS END --------------------//



//--------------------- LOGIC START ---------------------//


function getWkr() {
    return Math.floor(1E6 * Math.random()) + 1E3
}

function getRandomItem(array) {
    return array[Math.floor((Math.random() * array.length))]
}

function getTimestamp() {
    return parseInt((new Date).getTime(), 10)
}

function getShortTimestamp() {
    return parseInt((new Date).getTime() / 1E3, 10)
}

function hashWidget(a) {
    return a.replace(/[A-Za-z]/g, function(a) {
        return String.fromCharCode(a.charCodeAt(0) + ("M" >= a.toUpperCase() ? 13 : -13))
    })
}

function base64ToBytes(a) {
  a = atob(a);
  return Uint8Array.from(a, function (b) {
    return b.codePointAt(0);
  });
}

function bytesToBase64(a) {
  a = Array.from(a, function (b) {
    return String.fromCodePoint(b);
  }).join("");
  return btoa(a);
}
function nsgmdzxs(a) {
  var b = "";
  var c = [41, 8, 49, 48, 51, 44, 63, 0, 19, 61, 43, 63, 57, 15, 34, 6, 42, 59, 41, 19, 10, 45, 54, 0, 44, 34, 57, 36, 48];
  if (a.substring(0, 5) === "BNDX:") {
    a = a.substring(5);
    a = new TextDecoder().decode(base64ToBytes(a));
    a = a.substring(4);
    var e = true;
  } else {
    e = false;
    b = "NDX:";
  }
  var f = 0;
  for (var n = 0; n < a.length; n += 1) {
    var t = a.charCodeAt(n) - 32;
    if (t >= 0 && t < 94) {
      if (e && t < 64) {
        t ^= c[f % c.length];
      }
      t += 47 + (e ? -1 : 1) * f * 31;
      t = (t % 94 + 94) % 94;
      if (!e && t < 64) {
        t ^= c[f % c.length];
      }
      f++;
    }
    b += String.fromCharCode(t + 32);
  }
  if (e) {
    return b;
  } else {
    return "BNDX:" + bytesToBase64(new TextEncoder().encode(b));
  }
}

function quickHash(a) {
    var b = 0, c = 0, d;
    if (0 === a.length)
        return "00000000";
    var e = 0;
    for (d = a.length; e < d; e++) {
        var f = a.charCodeAt(e);
        0 === e % 2 ? (b = (b << 5) - b + f,
        b |= 0) : (c = (c << 5) - c + f,
        c |= 0)
    }
    0 > b && (b = 4294967295 + b + 1);
    0 > c && (c = 4294967295 + c + 1);
    return b.toString(16) + c.toString(16)
}

function updateIPR(a, d, val) {

    let e = getTimestamp()

    if(null == C) {
        val = val + b("ncip", e, [getShortTimestamp(), 2, 1]) + ";"
        C = e
    }

    val = val + b(a, e, d) + ";";

    15E3 <= e - aa && (val = val + b("ts", e, [e - fa]) + ";",
    aa = C = e);
  
  return val

}

function b(a, b, c) {
    b -= C;
    1 < 1 && (b = Math.round(b / 1));
    a = a + "," + b.toString(16);
    if (null != c && 0 < c.length) {
        a += ",";
        b = [];
        for (var d = 0; d < c.length; d++)
            "number" === typeof c[d] ? b.push(Math.round(c[d]).toString(16)) : null != c[d] && b.push(c[d].toString());
        c = b.join(",");
        a += c
    }
    return a
}

function randomRange(min, max) {
    return ~~(Math.random() * (max - min + 1)) + min
}

async function getWidgetData(type) {

    if(type == 'checkout-regular') {

        let nds = await axios.get('https://api-ticketmaster.nd.nudatasecurity.com/2.2/w/w-481390/sync/js/').then(e => e)
        
        let wt = nds.data.split('"wt":"')[1].split('"')[0]
        let version = nds.data.split('ndjsStaticVersion="')[1].split('"')[0]
        let wVersion = nds.data.split('ndsWidgetVersion="')[1].split('"')[0]
        let IPR = ''
                
        IPR = updateIPR("st", [""], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), '__next'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [randomRange(5000, 7000), 10, 'NOP'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), '__next'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mc", [randomRange(290, 300), randomRange(300, 310), 'placeOrderOptIn1input'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
                
        let data = {
            "widgetData": {
                "bd": getRandomItem(screens),                      //Screen Parameters
                "jsv": version,                                    //Script Version
                "wv": wVersion,                                    //Widget Version
                "bp": quickHash(getRandomItem(plugins)),           //Browser Plugins (Hashed)
                "sr": getRandomItem(fingerprints),                 //Screen Fingerprint
                "didtz": "420",                                      //Device Timezone
                "wkr": getWkr(),                                   //Random Value
                "flv": getRandomItem(["true", "false"]),               //Flash Installed?
                "fv": getRandomItem(video),                        //Supported Video Formats
                "fa": getRandomItem(audio),                        //Supported Audio Formats
                "hf": quickHash(getRandomItem(signatures)),        //Canvas Signature
                "pl": getRandomItem(platforms),                    //Device Platform
                "ft": getRandomItem(touch),                        //Device Touch Settings
                "fc": getRandomItem(cookie),                       //Cookies enabled?
                "fs": "true",                                      //HTML5LocalStrage
                "wg": quickHash(getRandomItem(webGL)),             //WebGL Information
                "fm": quickHash(getRandomItem(fonts)),             //Font Metrics
                "ipr": IPR,                                        //IPR Value
                "ic": "0,de;",                                     //Widget Values. "de" = ALL
                "af": "",                                          //Always null
                "dit": ""                                          //Always null
            },
            "wt": wt
        }

        return nsgmdzxs(JSON.stringify(data))

    }
            
    if(type == 'checkout-fanwallet') {

        let nds = await axios.get('https://api-ticketmaster.nd.nudatasecurity.com/2.2/w/w-481390/sync/js/').then(e => e)
        
        let wt = nds.data.split('"wt":"')[1].split('"')[0]
        let version = nds.data.split('ndjsStaticVersion="')[1].split('"')[0]
        let wVersion = nds.data.split('ndsWidgetVersion="')[1].split('"')[0]
        let IPR = ''
                
        IPR = updateIPR("st", [""], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mm", [randomRange(320, 340), randomRange(80, 120), ''], IPR)
                
        let data = {
            "widgetData": {
                "bd": getRandomItem(screens),                      //Screen Parameters
                "jsv": version,                                    //Script Version
                "wv": wVersion,                                    //Widget Version
                "bp": quickHash(getRandomItem(plugins)),           //Browser Plugins (Hashed)
                "sr": getRandomItem(fingerprints),                 //Screen Fingerprint
                "didtz": "420",                                      //Device Timezone
                "wkr": getWkr(),                                   //Random Value
                "flv": getRandomItem(["true", "false"]),               //Flash Installed?
                "fv": getRandomItem(video),                        //Supported Video Formats
                "fa": getRandomItem(audio),                        //Supported Audio Formats
                "hf": quickHash(getRandomItem(signatures)),        //Canvas Signature
                "pl": getRandomItem(platforms),                    //Device Platform
                "ft": getRandomItem(touch),                        //Device Touch Settings
                "fc": getRandomItem(cookie),                       //Cookies enabled?
                "fs": "true",                                      //HTML5LocalStrage
                "wg": quickHash(getRandomItem(webGL)),             //WebGL Information
                "fm": quickHash(getRandomItem(fonts)),             //Font Metrics
                "ipr": IPR,                                        //IPR Value
                "ic": "0,de;",                                     //Widget Values. "de" = ALL
                "af": "",                                          //Always null
                "dit": ""                                          //Always null
            },
            "wt": wt
        }

        return nsgmdzxs(JSON.stringify(data))

    }

    if(type == 'verifiedfan') {

        let nds = await axios.get('https://api-ticketmaster.nd.nudatasecurity.com/2.2/w/w-481390/sync/js/').then(e => e)
        
        let wt = nds.data.split('"wt":"')[1].split('"')[0]
        let version = nds.data.split('ndjsStaticVersion="')[1].split('"')[0]
        let wVersion = nds.data.split('ndsWidgetVersion="')[1].split('"')[0]
        let IPR = ''
                
        IPR = updateIPR("st", [""], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("st", ["email,42,first_name,6,last_name,5,zip,5,intl-phone-number,12,avionrewardsemail,42"], IPR)
        IPR = updateIPR("st", ["email,42,first_name,6,last_name,5,zip,5,intl-phone-number,12,avionrewardsemail,42"], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mm", [randomRange(320, 340), randomRange(80, 120), ''], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("kk", [0, 'avionrewardsemail'], IPR)
        IPR = updateIPR("ff", ['avionrewardsemail'], IPR)
        IPR = updateIPR("mc", [randomRange(260, 270), randomRange(265, 275), 'avionrewardsemail'], IPR)
        IPR = updateIPR("mm", [randomRange(320, 340), randomRange(80, 120), 'avionrewardsemail'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mc", [randomRange(260, 270), randomRange(265, 275), 'market'], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mc", [randomRange(260, 270), randomRange(265, 275), 'optional_markets-0'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mc", [randomRange(300, 310), randomRange(265, 275), ''], IPR)
                
        let data = {
            "widgetData": {
                "bd": getRandomItem(screens),                      //Screen Parameters
                "jsv": version,                                    //Script Version
                "wv": wVersion,                                    //Widget Version
                "bp": quickHash(getRandomItem(plugins)),           //Browser Plugins (Hashed)
                "sr": getRandomItem(fingerprints),                 //Screen Fingerprint
                "didtz": "420",                                      //Device Timezone
                "wkr": getWkr(),                                   //Random Value
                "flv": getRandomItem(["true", "false"]),               //Flash Installed?
                "fv": getRandomItem(video),                        //Supported Video Formats
                "fa": getRandomItem(audio),                        //Supported Audio Formats
                "hf": quickHash(getRandomItem(signatures)),        //Canvas Signature
                "pl": getRandomItem(platforms),                    //Device Platform
                "ft": getRandomItem(touch),                        //Device Touch Settings
                "fc": getRandomItem(cookie),                       //Cookies enabled?
                "fs": "true",                                      //HTML5LocalStrage
                "wg": quickHash(getRandomItem(webGL)),             //WebGL Information
                "fm": quickHash(getRandomItem(fonts)),             //Font Metrics
                "ipr": IPR,                                        //IPR Value
                "ic": "0,de;",                                     //Widget Values. "de" = ALL
                "af": "",                                          //Always null
                "dit": ""                                          //Always null
            },
            "wt": wt
        }

        return nsgmdzxs(JSON.stringify(data))

    }

}


//---------------------- LOGIC END ----------------------//



//-------------------- ROUTES START --------------------//

app.get('/login', async (req, res) => {

    axios.get('https://api-ticketmaster.nd.nudatasecurity.com/2.2/w/w-481390/sync/js/').then(response => {
      
        let wt = response.data.split('"wt":"')[1].split('"')[0]
        let version = response.data.split('ndjsStaticVersion="')[1].split('"')[0]
        let wVersion = response.data.split('ndsWidgetVersion="')[1].split('"')[0]
        
        let IPR = ''
        
        IPR = updateIPR("st", ["email[objectobject]__input,0,password[objectobject]__input,0"], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [0, 10, 'NOP'], IPR)
        IPR = updateIPR("mms", [randomRange(7000, 12000), 10, 'NOP'], IPR)
        IPR = updateIPR("kk", [0, 'email[objectobject]__input'], IPR)
        IPR = updateIPR("ff", ['email[objectobject]__input'], IPR)
        IPR = updateIPR("mc", [randomRange(260, 270), randomRange(265, 275), 'email[objectobject]__input'], IPR)
        IPR = updateIPR("mms", [randomRange(7000, 12000), 10, 'NOP'], IPR)
        IPR = updateIPR("kd", [0], IPR)  
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("fb", ['email[objectobject]__input'], IPR)
        IPR = updateIPR("kk", [0, 'password[objectobject]__input'], IPR)
        IPR = updateIPR("ff", ['password[objectobject]__input'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mms", [randomRange(5000, 7000), 10, 'NOP'], IPR)
        IPR = updateIPR("mc", [randomRange(275, 285), randomRange(285, 295), 'password[objectobject]__input'], IPR)
        IPR = updateIPR("kd", [1], IPR)  
        IPR = updateIPR("fb", ['password[objectobject]__input'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mc", [randomRange(290, 300), randomRange(300, 310), 'rememberMetrueinput'], IPR)
        IPR = updateIPR("mm", [randomRange(280, 320), randomRange(20, 65), ''], IPR)
        IPR = updateIPR("mc", [randomRange(290, 300), randomRange(300, 310), 'sign-in'], IPR)
        
        let data = {
            "widgetData": {
                "bd": getRandomItem(screens),                      //Screen Parameters
                "jsv": version,                                    //Script Version
                "wv": wVersion,                                    //Widget Version
                "bp": quickHash(getRandomItem(plugins)),           //Browser Plugins (Hashed)
                "sr": getRandomItem(fingerprints),                 //Screen Fingerprint
                "didtz": "420",                                      //Device Timezone
                "wkr": getWkr(),                                   //Random Value
                "flv": getRandomItem(["true", "false"]),               //Flash Installed?
                "fv": getRandomItem(video),                        //Supported Video Formats
                "fa": getRandomItem(audio),                        //Supported Audio Formats
                "hf": quickHash(getRandomItem(signatures)),        //Canvas Signature
                "pl": getRandomItem(platforms),                    //Device Platform
                "ft": getRandomItem(touch),                        //Device Touch Settings
                "fc": getRandomItem(cookie),                       //Cookies enabled?
                "fs": "true",                                      //HTML5LocalStrage
                "wg": quickHash(getRandomItem(webGL)),             //WebGL Information
                "fm": quickHash(getRandomItem(fonts)),             //Font Metrics
                "ipr": IPR,                                        //IPR Value
                "ic": "0,de;",                                     //Widget Values. "de" = ALL
                "af": "",                                          //Always null
                "dit": ""                                          //Always null
            },
            "wt": wt
        }

        return res.json(nsgmdzxs(JSON.stringify(data)))
        
    })

})

app.get('/checkout', async (req, res) => {

    let widgetOne = await getWidgetData('checkout-regular')
    let widgetTwo = await getWidgetData('checkout-fanwallet')

    return res.json({
        nuData: widgetOne,
        fanWalletNuData: widgetTwo
    })

})

app.get('/verifiedfan', async (req, res) => {

    let widgetData = await getWidgetData('verifiedfan')

    return res.json({
        nuData: widgetData
    })

})

//-------------------- ROUTES END --------------------//



server.listen(process.env.PORT || 1337, '0.0.0.0', () => {

    console.log('Server running on port ' + 1337)

})
